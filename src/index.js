...
const INTERFACE = {
  query(client, ...args) {
    ...
    promise.cancel = function cancel() {
      ...
      if (client.activeQuery === query) {
        return INTERFACE.query(client, "SELECT pg_cancel_backend($1)", [
          client.processID,
        ]);
      }
    };
  ...
  rows(...args) {
    return thenWithCancel(INTERFACE.query(...args), function (result) {
      return result.rows;
    });
  },
  row(...args) {
  ...
  value(...args) {
    return thenWithCancel(INTERFACE.row(...args), function (row) {
...
function withConnection(connection, work, cancellable) {
  ...
  if (cancellable) {
    promise.cancel = function () {
      ...
      if (
        activeWork !== null &&
        typeof activeWork === "object" &&
        typeof activeWork.cancel === "function"
      ) {
...
function configure(server) {
  ...
  if (
    (server.poolSize || process.env.PG_POOL_SIZE) &&
    typeof server.max === "undefined"
  ) {
  ...
  let iface = {
    connection(work) {
      return withConnection(connect(), function doConnection(client) {
        return work(
          Object.keys(INTERFACE).reduce(function linkInterface(i, methodName) {
